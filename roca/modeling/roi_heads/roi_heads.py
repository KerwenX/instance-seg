...
class ROCAROIHeads(StandardROIHeads):
    ...
    def forward(
        ...
        # visual = Visualizer(img_rgb=images[0].permute(1,2,0).cpu().numpy()[:,:,::-1],metadata=MetadataCatalog.get("Scan2CAD_val"),scale=1.5)
        del images
        if self.training:
            assert targets
        ...
        if self.training:
            ...
            # ========================================================================
            # '''
            # score_flt = [p.scores >= self.test_min_score for p in pred_instances]
            # instances = [p[flt] for p, flt in zip(pred_instances, score_flt)]
            # return instances
            # '''
            # =======================================================================
            # '''
            ...
            # vis_out = visual.draw_instance_predictions(predictions=instances[0].to('cpu'))
            # plt.figure()
            # plt.axis('off')
            # # plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=0, wspace=0)
            # plt.imshow(vis_out.get_image())
            # plt.show()
